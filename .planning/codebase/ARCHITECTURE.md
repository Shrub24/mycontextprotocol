...
**Namespaces:**
- Operators in system namespaces (`cnpg-system`, `dragonfly-system`, `keda`)
- App/data plane in `mycontextprotocol`

**Core Runtime Components:**
- CNPG cluster `mycontextprotocol-db` + app secret `<cluster>-app`
- Dragonfly instance `dragonfly`
- Gateway + worker (FastAPI + KEDA ScaledJob)
- LightRAG service (optional for graph queries)

## Secrets Model

- Internal DB credentials are generated by CNPG (`<cluster>-app`)
- Dragonfly password is chart-generated in `mycontextprotocol`
- External API keys are SOPS-encrypted and injected via helm-secrets
